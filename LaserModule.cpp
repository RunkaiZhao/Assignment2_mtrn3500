...
void laser::GetXYRangeData(System::String^ responsedata)
{
	...
	/*int index = 0;
	for (int j = 0; j < 50; j++) {
		if (stringArray[j] == "sRA") {
			index = j;
			break;
			//std::cout << "the loca of sRA " << index << std::endl;
		}
	}*/
	...
	if (stringArray[1] == "LMDscandata") {
		this->discard = 0;
		//array<System::String^>^ stringArray = responsedata->Split(' ');
		this->StartAngle = Convert::ToUInt32(stringArray[23], 16);
		this->Resolution = Convert::ToUInt16(stringArray[24], 16) / 10000;
		this->NumPoints = Convert::ToUInt16(stringArray[25], 16);
